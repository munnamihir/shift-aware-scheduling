...
def solve_shard(
    ...
    for loop in loops:
        for k, rounds in enumerate(loop.options):
            ...
            last_day, last_slot = max(rounds)
            # ASYNC DEBRIEF: any slot within 24h after the final round, rather than
            # the slot immediately following it. Tests whether relaxing debrief
            # timing alone frees tail-of-day cohorts under a contiguous protocol.
            cand = []
            for dd, ds in [(last_day, s) for s in range(last_slot + 1, DAY_SLOTS)] + \
                          [(last_day + 1, s) for s in range(0, last_slot + 1)]:
                if dd < 7:
                    cand.append((dd, ds))
            cand = cand[:12]
            if not cand:
            ...
            db = {}
            for (dd, ds) in cand:
                db[(dd, ds)] = model.NewBoolVar(f"db_{loop.id}_{k}_{dd}_{ds}")
            model.Add(sum(db.values()) == wk)
            ...
            for r, (day, slot) in enumerate(rounds):
                ...
                for person in pool:
                    ...
                    # A panelist can only be assigned if they are free at whichever
                    # debrief slot is chosen.
                    for (dd, ds), dv in db.items():
                        if ds not in person.available_slots(dd):
                            model.Add(av + dv <= 1)
